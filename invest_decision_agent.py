...
class MarketState(TypedDict, total=False):
    tam_usd_b: Optional[float]
    cagr_pct: Optional[float]
    problem_fit_text: Optional[str]
    demand_drivers: List[str]
    problem_fit_score_0to5: Optional[int]  # LLM (0~5)


class TechnologyState(TypedDict, total=False):
    technology_summary: Optional[str]
    core_technology: Optional[str]
    sota_performance_note: Optional[str]  # e.g. "+5%" text allowed
    reproduction_difficulty: Optional[str]  # "low" | "mid" | "high" | etc.
    infrastructure_requirements: List[str]
    ip_patent_status: Optional[str]  # free text OK (e.g., "3건 출원 1건 등록")
    scalability_note: Optional[str]
    tech_risks_texts: List[str]
    perf_delta_pct: Optional[float]  # if available
    speed_delta_pct: Optional[float]  # if available
    churn_pct: Optional[float]
    csat_pct: Optional[float]
    # LLM checklist (0/1)
    checklist_api: Optional[int]
    checklist_multi_tenancy: Optional[int]
    checklist_sdk_docs: Optional[int]
    checklist_automation: Optional[int]
    checklist_domain_extensibility: Optional[int]


class CompetitorRecord(TypedDict, total=False):
    name: str
    overlap_0to10: Optional[float]
    differentiation_0to10: Optional[float]
    moat_0to10: Optional[float]
...
class CompetitionState(TypedDict, total=False):
    ...
    qual_positioning_score_0to5: Optional[int]  # LLM (0~5)


class BusinessState(TypedDict, total=False):
    arr_usd_m: Optional[float]  # millions
...
class DecisionBlock(TypedDict, total=False):
...
def normalize_input(raw: Dict[str, Any]) -> GraphState:
    state: GraphState = {}
...
def score_market(market: MarketState) -> float:
    if not market:
        return 60.0
...
def score_technology(tech: TechnologyState) -> float:
    if not tech:
        return 60.0
...
def score_competition(comp: CompetitionState) -> float:
    if not comp:
        return 60.0
...
def score_traction(biz: BusinessState) -> float:
    if not biz:
        return 60.0
...
def aggregate_scores(state: GraphState) -> GraphState:
    comp_scores: Dict[str, ComponentScore] = {
