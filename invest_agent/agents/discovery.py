...
def startup_discovery(state: GraphState) -> GraphState:
    """
    Workflow용 래퍼: 기업 탐색
    
    입력: state["query"]
    출력: state["discovery"], state["companies"]
    """
...
def pick_company(state: GraphState) -> GraphState:
    """
    Workflow용 래퍼: 현재 회사 선택
    
    입력: state["companies"], state["idx"]
    출력: state["current_company"]
    """
