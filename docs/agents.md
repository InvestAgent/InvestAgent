...
이전 에이전트 결과 요약 → 검색 질의 생성 → 웹 검색 → LLM 분석 요약 → 데이터 보완

### 1️⃣ 입력 요약 (Summarization / Query Expansion)

입력: 이전 에이전트의 JSON 결과
LLM이 이전 에이전트 내용의 핵심만 요약
기술 중심의 검색 질의 생성

2️⃣ 웹 검색 (Retrieval)

생성된 기술 키워드로 최신 논문, 기술 리포트, 특허 관련 문서 검색
기술 성능 개선, 구조 변경, 최신 버전 정보 등 탐색

3️⃣ 검색 결과 요약 (Compressor)

검색된 웹 콘텐츠를 LLM이 다시 요약
LLM이 검색 결과에서 기술적 사실만 추출


4️⃣ LLM 정보 통합 및 구조화 (Generation)

이전 요약 결과 + 새 검색 요약을 결합
LLM이 최신 기술 정보를 반영한 JSON 출력 생성


5️⃣ 데이터 보완 (Post-processing)
